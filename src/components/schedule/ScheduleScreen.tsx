    ...
    <div className="screen">
        ...
        return (
          <section key={i} className="schedule-round">
            ...
            <div className="schedule-groups">
              {r.groups.map((g) => (
                <GroupCard key={g.id} group={g} compact onClick={readOnly ? undefined : () => setEditing({ group: g, context: `Round ${i + 1}` })} />
              ))}
            </div>
      ...
      {t.final?.kind === 'bracket' && t.final.bracket && (
        <section className="schedule-round">
          ...
          <BracketRounds bracket={t.final.bracket} compact onEdit={readOnly ? undefined : (group, context) => setEditing({ group, context, final: true })} />
        </section>
      )}
        ...
        <section className="schedule-round">
          ...
          <div className="schedule-groups">
            {t.final.groups.map((g, i) => (
              <GroupCard key={g.id} group={g} compact seeded onClick={readOnly ? undefined : () => setEditing({ group: g, context: tierName(i, t.final!.groups.length), final: true })} />
            ))}
          </div>
        </section>
      )}
